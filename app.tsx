...
import { MapView, AmbientLight, PointLight, LightingEffect } from '@deck.gl/core';
...
// Source data GeoJSON
const DATA_URL =
  'https://kumiko-haraguchi.github.io/data-store/flood/floodArchive.geojson'; // eslint-disable-line
// import DATA_URL from "./FloodArchive.csv";
// const DATA_URL =
...
function getTimeRange(features?: Feature<Geometry, GeoJsonProperties>[]): [minTime: number, maxTime: number] | null {
  if (!features) {
    return null;
  }
...
function getTooltip({ object }: PickingInfo<Feature<Geometry, GeoJsonProperties>>) {
  ...
  const { properties: { Dead, Area, timestamp } } = object;
...
export default function App({
  ...
  const layers = [
    ...
    new GeoJsonLayer<GeoJsonProperties>({
      ...
      getPosition: (f: Feature<Geometry, PropertiesType>) => f.geometry?.coordinates,
      getPointRadius: (f: Feature<Geometry, PropertiesType>) => Math.sqrt(f.properties.Area) * 100,
